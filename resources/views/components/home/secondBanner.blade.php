<div class="w-full h-[35vh] flex flex-col justify-center items-center pt-[70px]"
    style="background: url(data:image/png;base64,) left top repeat-x rgb(247, 247, 247);">

    <div class="w-[80%] flex gap-[50px] justify-around">
        <div class="w-[31%] h-[180px] flex relative">
            <div
                class="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[98%] h-[110%] border-[1px] border-[#ccc]">
            </div>
            <div class="w-[30%] h-full">
                <img class="object-cover w-full h-full"
                    src="https://images.contentstack.io/v3/assets/blt731acb42bb3d1659/bltcc2e76f9f2459cfb/64b5cc84db581da632060165/Riot_Client_T2-NoText.jpg?quality=90&crop=1%3A1&width=240"
                    alt="11">
            </div>
            <div class="flex-1 h-full px-[10px]">
                <h3 class="uppercase text-[11px] text-[#999] font-bold">Cập nhật trò chơi</h3>
                <h2 class="block my-[10px] font-bold text-[17px]">Chiến binh tận thế vô địch thế giới | Giới thiệu DRX
                    Aatrox Tí Nị
                    ft. Kingen
                </h2>
                <p class="text-[12px]">Cùng luyện tập với Kingen để chuẩn bị cho sự xuất hiện của DRX Aatrox Tí Nị –
                    Linh Thú đầu tiên được
                    lấy cảm hứng từ Nhà Vô Địch Thế Giới của chúng tôi, hiện đã được ra mắt!</p>
            </div>
        </div>

        <div class="w-[31%] h-[180px] flex relative">
            <div
                class="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[98%] h-[110%] border-[1px] border-[#ccc]">
            </div>
            <div class="w-[30%] h-full">
                <img class="object-cover w-full h-full"
                    src="https://images.contentstack.io/v3/assets/blt731acb42bb3d1659/bltd44a9347457d90fb/64b5e4d974dae3b01e282b23/00HeaderChoncc.jpg?quality=90&crop=1%3A1&width=240"
                    alt="11">
            </div>
            <div class="flex-1 h-full px-[10px]">
                <h3 class="uppercase text-[11px] text-[#999] font-bold">Đội ngũ phát </h3>
                <h2 class="block my-[10px] font-bold text-[17px]">Đằng Sau Chế Độ Chơi: Tinh Võ Đấu Trường
                </h2>
                <p class="text-[12px]">Đã tới phần tập luyện rồi.</p>
            </div>
        </div>

        <div class="w-[31%] h-[180px] flex relative">
            <div
                class="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[98%] h-[110%] border-[1px] border-[#ccc]">
            </div>
            <div class="w-[30%] h-full">
                <img class="object-cover w-full h-full"
                    src="https://images.contentstack.io/v3/assets/blt731acb42bb3d1659/blt5014281deaaf84ae/64adb4be7b26df1639e17c7b/Primetime_Draven_Splash.jpg?quality=90&crop=1%3A1&width=240"
                    alt="11">
            </div>
            <div class="flex-1 h-full px-[10px]">
                <h3 class="uppercase text-[11px] text-[#999] font-bold">Cộng đồng</h3>
                <h2 class="block my-[10px] font-bold text-[17px]">Tinh Võ Sư: Đêm Tỉ Thí hiện đã MỞ để Đồng Phát Sóng!
                </h2>
                <p class="text-[12px]">Thông tin chi tiết về cách bạn đồng phát sóng Tinh Võ Sư: Đêm Tỉ Thí vào ngày
                    18/07</p>
            </div>
        </div>
    </div>
    <div class="w-[80%] h-[50px] flex justify-end">
        <button class="flex items-center">
            <p class="uppercase font-medium text-[12px] mr-[10px]">Xem tất cả</p>
            <i class="fas fa-arrow-right text-[rgb(11,198,227)]"></i>
        </button>
    </div>
</div>